...
export default function ArrangementDetailPage() {
  ...
  const limits = meData?.subscription?.limits;
  const isFreeTier = meData?.subscription?.tier === "free";
  const allowPracticeTools = limits?.allowPracticeTools === true;
  const allowSyncMaps = limits?.allowSyncMaps === true;
  const allowMusicXml = limits?.allowMusicXml === true;
  ...
  // Transcription is enabled by default; set NEXT_PUBLIC_TRANSCRIPTION_ENABLED=false to kill-switch it.
  // Tier gating is enforced server-side via requireFeature("allowAiProcessing").
  const transcriptionEnabled = process.env.NEXT_PUBLIC_TRANSCRIPTION_ENABLED !== "false";
  ...
  // Guard: wait for arrangement data to load before deciding whether to auto-generate,
  // otherwise a race between readiness and arrangement queries can trigger a false run.
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [isLoading, hasAudio, hasSyncMap, hasSections, isGeneratingSections]);
  ...
  function renderUpgradePrompt(featureLabel: string) {
    return (
      <Flex
        align="center"
        gap={2}
      ...
      >
        ...
        <Box flex={1}>
          <Text fontSize="xs" color="gray.500">
            {featureLabel} requires a{" "}
            <Text as="span" fontWeight="bold" color="blue.500">Band</Text>{" "}
            plan or higher
          </Text>
        </Box>
        <Button size="xs" variant="outline" colorPalette="blue" onClick={() => router.push("/pricing")}>
          Upgrade
        </Button>
      </Flex>
    );
  ...
  function renderChartsStepExtra() {
    ...
    return (
      <VStack align="stretch" gap={2}>
        {!allowMusicXml && hasStems && (
          renderUpgradePrompt("AI sheet music generation")
        )}
        {transcriptionError && (
        ...
        {allowMusicXml && transcriptionEnabled && hasCharts && stemsWithCharts.length > 0 && !isTranscribing && !isRegenerating && (
  ...
  function renderSectionsStepExtra() {
    ...
    return (
      <VStack align="stretch" gap={2}>
        {!allowMusicXml && canGenerate && (
          renderUpgradePrompt("AI section generation")
        )}
        {allowMusicXml && canGenerate && !isGeneratingSections && !sectionGenError && (
          <Flex align="center" gap={2} p={3} borderRadius="md" bg="purple.50" border="1px solid" borderColor="purple.100">
            <Box flex={1}>
              <Flex align="center" gap={1.5}>
          ...
          <Dialog.Content maxW="500px">
            ...
            <Dialog.Body>
              <VStack gap={4} align="stretch">
                {/* AI Generate option */}
                {!allowMusicXml && stemsForTranscription.length > 0 && (
                  <Box>
                ...
                {allowMusicXml && transcriptionEnabled && stemsForTranscription.length > 0 && !isTranscribing && (
                  <Box>
                    <Text fontSize="xs" fontWeight="semibold" color="gray.500" textTransform="uppercase" mb={2}>
                      Generate with AI
                    </Text>
